...
namespace ThrownDaggers.Extensions
{
    static class ExtentionMethods
    {
        ...
        [CanBeNull]
        public static T GetAction<T>([CanBeNull] this ActionList actions)
        {
            if (actions == null)
            {
                return default(T);
            }
            for (int index = 0; index < actions.Actions.Length; index++)
            {
                GameAction action;
                if ((action = actions.Actions[index]) is T)
                {
        ...
        public static void AddVendorItem(this BlueprintSharedVendorTable table, string guid, int count)
        {
            table.AddComponent<LootItemsPackFixed>(loot =>
            {
                loot.m_Item = Helpers.Create<LootItem>(item =>
                {
                    item.m_Item = Resources.GetBlueprint<BlueprintItem>(guid).ToReference<BlueprintItemReference>();
                    item.m_Type = LootItemType.Item;
                });
                loot.m_Count = count;
            });
        }

        public static void AddToBlueprintLoot(this BlueprintLoot loot, string guid, int count)
        {
            loot.Items = loot.Items.Append(new LootEntry() { Count = count, Item = Resources.GetBlueprint<BlueprintItem>(guid) }).ToArray();
        }
    }
}
